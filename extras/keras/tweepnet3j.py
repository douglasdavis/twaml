...
def prepare_data(branchinfo_file="../vars.yaml", region="3jHb"):
    with open(branchinfo_file, "r") as f:
        branches = yaml.load(f, Loader=yaml.FullLoader)
    branches = branches[region]

    ttbar = from_pytables(f"/home/ddavis/ATLAS/data/h5s/ttbar_r{region}.h5", label=0, auxlabel=1)
    tW_DR = from_pytables(f"/home/ddavis/ATLAS/data/h5s/tW_DR_r{region}.h5", label=1, auxlabel=1)
...
def simple_model_sgd(inshape, nlayers=8, nnode=64, lr=1.0e-5, momentum=0.7):
...
def train_model(
    model, X_train, y_train, w_train, X_test, y_test, w_test, output_dir=None, epochs=100
):
    scaler = RobustScaler()
...
def main():
    ...
    for i, (test_idx, train_idx) in enumerate(folder.split(X)):
        ...
        false_pos, true_pos, thresh = roc_curve(y_test, y_pred, sample_weight=w_test)
        fig, ax = plt.subplots()
